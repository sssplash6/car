...
import { IkatDivider, Rosette } from "@/app/_components/Ornament";
    ...
    <div className="mx-auto w-full max-w-6xl px-6 py-12">
      ...
      {query ? (
        ...
        [...byYear.entries()].map(([year, yearRows], i) => (
          <section
            key={year}
            className={`grid gap-x-12 gap-y-2 py-10 lg:grid-cols-[10rem_1fr] ${
              i > 0 ? "border-t border-rule-strong" : ""
            }`}
          >
            {/* Year numeral in ink, not accent: display, not action — the same
                rule that governs the issue numerals. */}
            <h2 className="oldstyle-nums font-serif text-[2.5rem] leading-none tracking-tight text-ink">
              {year || "Undated"}
            </h2>
            <div>
              {yearRows.map(({ paper, image }) => (
                <Reveal key={paper.id}>
